...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "set_city",
            "description": "Save the user's preferred city so future weather/briefing uses it. Use when the user supplies a city name as their place.",
            "parameters": {
                "type": "object",
                "properties": {"city": {"type": "string", "description": "City name, e.g. Malang."}},
                "required": ["city"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "get_city",
            "description": "Return the user's saved preferred city (empty if never set).",
            "parameters": {
                "type": "object",
                "properties": {},
                "required": [],
...
def _identity_block() -> str:
    """JARVIS identity block from utils/identity.py (single source of truth)."""
    try:
        from utils.identity import SYSTEM_PROMPT_IDENTITY_BLOCK
        return SYSTEM_PROMPT_IDENTITY_BLOCK
    except Exception:
        return (
