...
def _run_pipeline(task_id: str, telegram_id: int, user_input: str,
    ...
    # SELF-REFERENTIAL INTERCEPT — the first and most important guard.
    # If the user asks "siapa kamu" / "apa yang bisa kamu lakukan" (or the
    # English equivalents), answer directly from identity.py — the single
    # source of truth. NEVER route this to the LLM, because a generic model
    # will hallucinate (e.g. answer about "uang"/money). This guard runs at
    # the top of the pipeline, before any planner/tool/search is triggered.
    ...
    if is_self_referential(user_input):
        try:
            ...
            supabase_client.insert_chat(telegram_id, "assistant", SELF_REF_REPLY)
        except Exception:
            pass
        supabase_client.update_task(task_id, {
            "status": "DONE",
            "result_text": SELF_REF_REPLY[:4000],
            "tool_calls": [],
        })
        _notify_user(telegram_id, SELF_REF_REPLY,
...
def _dispatch_tool(name: str, args: dict, telegram_id: int = None):
    ...
    if name == "set_city":
        ...
        return prefs.set_city(telegram_id, args.get("city", ""))
    if name == "get_city":
        from utils import prefs
        city = prefs.get_city(telegram_id)
