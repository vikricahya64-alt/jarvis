...
def send_daily_briefing(telegram_id: int = _DEFAULT_TG, city: str = None):
    """Compose and send the daily briefing to a user. Uses the user's saved
    city when set (see utils.prefs); falls back to Jakarta."""
    if not city:
        from utils import prefs
        city = prefs.get_city(telegram_id) or "Jakarta"
    text = _fetch(city)
