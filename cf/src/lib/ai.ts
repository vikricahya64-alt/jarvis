...
export async function searchAndSynthesize(
  ...
  const generative = await groqRespond(env, userText, { context, topic });
  if (generative) {
    await appendMemory(env, owner, "user", userText, topic);
    await appendMemory(env, owner, "assistant", generative, topic);
    return { reply: generative, source: "groq+ddg" };
  }
  if (searchResult) {
